    ...
    async def message(self, message: discord.Message) -> bool:
        ...
        if message_int > self.nb:
            ...
            await message.channel.send(
                f"Bien joué ! Mon nombre est bien : {message_int}"
            )
...
class MatBot(commands.Bot):
    ...
    def load_data(self):
        ...
        for user in users_keys.keys():
            self.cript_tables[self.cript.translate(user)] = CriptTable(
                self.cript.translate(users_keys[user])
            )

    def save_data(self):
        user_keys = {}
        for user in self.cript_tables.keys():
            user_keys[self.cript.translate(user)] = self.cript.translate(
                self.cript_tables[user].seed
            )
    ...
    def have_permissions(self, *perms: discord.flags.flag_value):
        async def predicate(ctx: discord.ApplicationContext):
            for perm in perms:
                if not (
                    ctx.user.guild_permissions._has_flag(perm.flag)
                    or ctx.user.id in self.admin
                ):
                    await ctx.channel.send(
                        "Vous n'avez pas assez de droits pour executer cette commande !"
                    )
                    return False
            return True
    ...
    def on_interaction(self, interaction: discord.interactions.Interaction):
        ...
        print(
            f"{interaction.guild.name} -> {interaction.channel.name} -> {interaction.user.display_name}: {command}"
        )
    ...
    async def on_reaction_add(self, reaction, user):
        for jeu in self.jeu:
            if (
                jeu.id_user == user.id
                and jeu.id_message == reaction.message.id
                and reaction.message.channel.id == jeu.channel.id
            ):
...
@bot.slash_command(name="set_key", description="Set your key")
async def set_key(ctx: discord.ApplicationContext, key: str):
    ...
    await response(
        ctx,
        title=f"Your key has been is set to : ||{key}||",
        embed=True,
        ephemeral=True,
    )
...
async def view_key(ctx: discord.ApplicationContext):
    if str(ctx.user.id) not in bot.cript_tables:
        await response(
            ctx,
            title="You haven'y a key",
            content="You can set one with `/set_key [key]`",
            embed=True,
            ephemeral=True,
        )
        return
...
async def del_key(ctx: discord.ApplicationContext):
    if str(ctx.user.id) not in bot.cript_tables:
        await response(
            ctx,
            title="You haven'y a key, nothing to delete...",
            embed=True,
            ephemeral=True,
        )
        return
    ...
    await response(
        ctx, ephemeral=True, embed=True, title=f"Your key ||{key}|| has been delete"
    )


@bot.slash_command(name="translate", description="Translate a text with your key")
...
@bot.slash_command(name="jouer", description="Joue au nombre mystère")
async def jouer(ctx: discord.ApplicationContext):
    msg = await response(ctx, title="Voulez vous jouer au nombre mystère ?", embed=True)
    await msg.add_reaction("✅")
    await msg.add_reaction("❌")
    bot.jeu.append(
        NombreMystere(id_user=ctx.user.id, id_message=msg.id, channel=ctx.channel)
    )


@bot.slash_command(name="research", description="Fait une recherche google")
async def research(
    ctx: discord.ApplicationContext, number_of_results: int, *, recherche: str
):
    ...
    try:
        liens = list(
            search(recherche, num_results=number_of_results, lang="fr", timeout=5)
        )
    except HTTPError:
        await response(
            ctx,
    ...
    await response(
        ctx,
        embed=True,
        title=f"Voici ce que j'ai trouvé pour '{recherche}' :",
        content=liste,
    )
...
async def alea(ctx: discord.ApplicationContext):
    await response(
        ctx,
        embed=True,
        title=f"{random.choice(bot.pronom)} {random.choice(bot.verbe)} {random.choice(bot.gn)}.",
    )


@bot.slash_command(
    name="pronom", description="Ajoute un pronom pour la création de phrase aléatoire"
)
async def add_pronom(ctx: discord.ApplicationContext, *, pronom: str):
    bot.pronom.append(pronom)
    await response(
        ctx,
        title=f"Le pronom '{pronom}' à bien été ajouté !",
        embed=True,
        ephemeral=True,
    )
...
@bot.slash_command(
    name="verbe", description="Ajoute un verbe pour la création de phrase aléatoire"
)
async def add_verbe(ctx: discord.ApplicationContext, *, verbe: str):
    bot.verbe.append(verbe)
    await response(
        ctx, title=f"Le verbe '{verbe}' à bien été ajouté !", embed=True, ephemeral=True
    )


@bot.slash_command(
    name="gn",
    description="Ajoute un groupe nominal pour la création de phrase aléatoire",
)
async def add_gn(ctx: discord.ApplicationContext, *, gn: str):
    bot.gn.append(gn)
    await response(
        ctx, title=f"Le gn '{gn}' à bien été ajouté !", embed=True, ephemeral=True
    )


@bot.slash_command(
    name="random_emoji",
    description="Ajoute des émojis aléatoires sous le dernier message",
)
...
async def random_emoji(ctx: discord.ApplicationContext, number: int = 1):
    ...
    async for message in ctx.channel.history(limit=1):
        ...
        await response(
            ctx,
            embed=True,
            title=f"Ajout de {x} émoji(s) sur le message de {message.author.display_name}",
        )


@bot.slash_command(
    name="add_emoji", description="Ajoute un émoji sous le dernier message"
)
@bot.have_permissions(PERMISSIONS.add_reactions, PERMISSIONS.manage_emojis)
async def add_emoji(ctx: discord.ApplicationContext, emoji):
    ...
    async for message in ctx.channel.history(limit=1):
        ...
        await response(
            ctx,
            embed=True,
            title=f"Ajout de l'émoji {emoji} sous le message de {message.author.display_name}",
            ephemeral=True,
        )
...
async def clear(ctx: discord.ApplicationContext, number: int):
    ...
    await response(
        ctx,
        title=f"{number} message(s) ont bien été supprimés",
        embed=True,
        ephemeral=True,
    )
...
@bot.slash_command(
    name="ban", description="Fait en sorte qu'un utilisateur ne puisse plus écrire"
)
...
async def ban(ctx: discord.ApplicationContext, user: discord.Member):
    if user.id in bot.ban:
        await response(
            ctx,
            title=f"Le membre {user.display_name} est déjà banni.",
            embed=True,
            ephemeral=True,
        )
    else:
        ...
        await response(
            ctx, title=f"Le membre {user.display_name} à été banni !", embed=True
        )


@bot.slash_command(
    name="unban", description="Fait en sorte qu'un utilisateur ne sois plus banni"
)
@bot.have_permissions(PERMISSIONS.mute_members)
async def unban(ctx: discord.ApplicationContext, user: discord.Member):
    if user.id in bot.ban:
        ...
        await response(
            ctx, title=f"Le membre {user.display_name} à été débanni !", embed=True
        )
    else:
        await response(
            ctx,
            title=f"Le membre {user.display_name} n'est pas banni.",
            embed=True,
            ephemeral=True,
        )
...
async def logs(ctx: discord.ApplicationContext, numbers: int = 20):
    if numbers > 500:
        await response(
            ctx, f"Le nombre maximum de commandes est 500...", ephemeral=True
        )
        return
    await defer(ctx)
    ...
    await response(
        ctx,
        embed=True,
        title=f"Historique des {numbers} dernières commandes :",
        content=logs,
    )
